...
def get_face_embedding(model_path,files_list, names_list):
    ...
    colorSpace="RGB"
    # 初始化mtcnn人脸检测
    face_detect = face_recognition.Facedetection() #加载检测模型对象
    # 初始化facenet
    face_net = face_recognition.facenetEmbedding(model_path) #加载特征提取模型结构对象
    ...
    for image_path, name in zip(files_list, names_list):
        ...
        bboxes, landmarks = face_detect.detect_face(image)  #检测人脸（非正方形）
        bboxes, landmarks =face_detect.get_square_bboxes(bboxes, landmarks,fixed="height") #检测人脸正方形
        ...
        if len(bboxes) >= 2 or len(landmarks) >= 2:
            ...
            continue
        # 获得人脸区域
        face_images = image_processing.get_bboxes_image(image, bboxes, resize_height, resize_width) #获取人脸图像
        # 人脸预处理，归一化
        face_images = image_processing.get_prewhiten_images(face_images,normalization=True) #人脸图像标准化
        # 获得人脸特征
        pred_emb = face_net.get_embedding(face_images) #使用facenet模型提取人脸特征embedding编码
...
def create_face_embedding(model_path,dataset_path,out_emb_path,out_filename):
    ...
    np.save(out_emb_path, embeddings) #保存头像的embedding编码矩阵数据到文件···
    file_processing.write_list_data(out_filename, label_list, mode='w') #保存头像编码矩阵数据对应的头像分类列表（头像人名列表）
