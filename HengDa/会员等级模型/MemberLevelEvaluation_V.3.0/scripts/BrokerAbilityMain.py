...
class evaluate_broker_data:
    ...
    def null_broker_score(self, update_time):
        ...
        # print(self.broker_entropy_value_df['总分'].max())
    ...
    def save_pickle_entropy_info(self, filename='feature_weight', save_path='./'):
    ...
    def sklearn_broker_cluster(self, broker_level_score_df, cluster_model='Kmeans'):
        ...
        if cluster_model == 'Kmeans':
            ...
            # 保存Kmears模型
            if 'BrokerKmeansModel.pkl' not in os.listdir('./'): #如果模型不存在时···
                # 训练模型
                ...
                joblib.dump(KM_model, 'BrokerKmeansModel.pkl')  # 保存模型···
            else:
                KM_model = joblib.load('BrokerKmeansModel.pkl')
            ...
            broker_level_score_df.groupby('BrokerLevelFlag').describe().to_excel(
                r'./sklearn_broker_cluster_describe_info.xlsx')  # 会员聚类分群数据分布特征···
