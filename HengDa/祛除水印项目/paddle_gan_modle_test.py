...
def output_image(data,save_picture_name):
    from PIL import Image
    new_im = Image.fromarray(data)  # data二维图片矩阵。
    new_im = new_im.resize((108,36))
    new_im.save(save_picture_name)
    # new_im.show()

def test(picture_name):
    im_path = 'water_picture_program_model/yes_water_picture/'+picture_name
    IMAGE_SIZE = 128
    # 原始数据
    x = fluid.layers.data(name='x_water', shape=[IMAGE_SIZE, IMAGE_SIZE, 3], dtype='float32')
    # 读图
    x = cv2.imread(im_path)
    x = cv2.resize(x, (IMAGE_SIZE, IMAGE_SIZE))
    x = cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
    x = np.array([x / 127.5 - 1], dtype=np.float32)
    ...
    img = np.array((completion_test[0] + 1) * 127.5, dtype=np.uint8)
    # print(img.shape)
    save_picture_name = './remove_mask/'+picture_name
    output_image(img,save_picture_name)

if __name__ == '__main__':
    all_remove_picture = read_pictrue_name(pictrue_father_path = './water_picture_program_model/yes_water_picture/')
    for picture_name in all_remove_picture:
        print(picture_name)
        try:
            test(picture_name)
        except:
